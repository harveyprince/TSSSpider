	...
	var Constructer = function(){
		this.eventInit = function($holder,options){
			...
			if(options.onloginformsubmit){
				$holder.find("form[type='登录']").submit(function(e){
					options.onloginformsubmit(e,this);
				});
			}
			if(options.onsignupformsubmit){
				$holder.find("form[type='注册']").submit(function(e){
					options.onsignupformsubmit(e,this);
				});
			}
		};
				...
				$(this).parents('form').find('input').each(function(){
					if($(this).attr('required')){
						if(($(this).val()!="")&&($(this).val()!=null)){
							...
							$box.alarm($(this).attr('placeholder') + " 不能为空!");
							e.preventDefault();
						}
					}
				});
		...
		this.createBox = function($holder,options){
			...
			if(options.loginAble){
				...
				var content = [];
				content.push(options.signupAble?createSwitchNav('登录','注册'):"");
				content.push(createForm(options.loginAction,'登录',inputHTML));
				var group = createGroup('登录',content.join(""),active.pop());
				box.push(group);
			}
			if(options.signupAble){
				...
				var inputHTML = inputcontent.join("");
				var content = [];
				content.push(options.loginAble?createSwitchNav('注册','登录'):"");
				content.push(createForm(options.signupAction,'注册',inputHTML));
				var group = createGroup('注册',content.join(""),active.pop());
				box.push(group);
			}
		...
		var createInput = function(type){
			...
			switch(type){
				case 'name':
				inputcontent.push('<input class="form-box-input name-input" placeholder="姓" type="text" name="surname" required="required">');
				inputcontent.push('<input class="form-box-input name-input" placeholder="名" type="text" name="psname" required="required">');
				break;
				case 'email':
				inputcontent.push('<span class="fa fa-user"></span>');
				inputcontent.push('<input class="form-box-input" placeholder="邮箱" type="email" style="ime-mode:disabled;" name="email" required="required">');
				break;
				case 'password':
				inputcontent.push('<span class="fa fa-key"></span>');
				inputcontent.push('<input class="form-box-input" placeholder="密码" type="password" name="password" required="required">');
				break;
